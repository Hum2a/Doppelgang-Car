...
methods: {
  handleFileInput(event) {
    ...
    console.log(`[INFO] Selected file: ${this.selectedFile.name}`);
  },
  async uploadImage() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`[ERROR] Network response failed with status: ${response.status}`);
      }

      const data = await response.json();
      console.log(`[INFO] Response received: ${JSON.stringify(data)}`);

      if (data.error) {
        throw new Error(`[ERROR] Backend returned an error: ${data.error}`);
      }
